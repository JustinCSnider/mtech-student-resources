...
if let unwrappedArea = calculateArea(x: 5, y: 3) {
    print(unwrappedArea)
}
...
func add(_ num1: Int?, _ num2: Int?) -> Int? {
    guard let unwrappedNum1 = num1, let unwrappedNum2 = num2 else {
        return nil
    }
    return unwrappedNum1 + unwrappedNum2
}

if let unwrappedAdd = add(12, 3) {
    print(unwrappedAdd)
}
if let unwrappedAdd = add(nil, 2) {
    print(unwrappedAdd)
}
/*:
...
func createUser() -> User? {
    guard let unwrappedFNT = firstNameTextField.text, let unwrappedLNT = lastNameTextField.text, let unwrappedATF = ageTextField.text else {
        return nil
    }
    
    return User(firstName: unwrappedFNT, lastName: unwrappedLNT, age: unwrappedATF)
}
/*:
...
if let myUser = createUser() {
    print("\(myUser.firstName)'s last name is \(myUser.lastName) and \(myUser.firstName)'s age is \(myUser.age).")
}
