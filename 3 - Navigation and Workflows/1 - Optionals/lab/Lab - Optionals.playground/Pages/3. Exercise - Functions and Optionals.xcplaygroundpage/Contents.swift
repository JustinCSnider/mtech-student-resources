...
func checkAge(userAge: String) -> Int? {
    if let unwrappedUserAge = Int(userAge){
        if unwrappedUserAge > 18 {
            print("Welcome!")
        } else {
            print("Sorry, but you aren't old enough to use our app.")
        }
        return unwrappedUserAge
    } else {
...
func checkStock(stockItem: String) -> Double? {
    if let unwrappedStockValue = stock[stockItem] {
        if unwrappedStockValue == 0 {
            print("We don't have any \(stockItem)'s in stock right now.")
        } else {
            if let unwrappedPriceValue = prices[stockItem] {
                print("The price of \(stockItem)'s is \(unwrappedPriceValue).")
                return unwrappedPriceValue
            }
        }
    } else {
        print("We don't carry \(stockItem)'s")
    }
    return nil
}

checkStock(stockItem: "Apple")
